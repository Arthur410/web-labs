...
export class BrokersGateway {
  ...
  updateBroker(@MessageBody() newBroker: IBroker): WsResponse<string> {
  ...
  sellStock(@MessageBody() passedData: {
    brokerName: string,
    stockToOwn: IOwnedStock
  }): WsResponse<string> {
    const broker = this.brokers.filter(broker => broker.name === passedData.brokerName)[0];

    console.log(broker, 'продает акцию', passedData.stockToOwn);

    return { event: 'buyStock', data: 'success' };
  }
}